...
Замер времени происходил в Docker-контейнере с запуском команды mpirun -n N ./ppc_perf_tests --gtest_filter="* KlimenkoVVMultistep2DSad *", где N - количество процессов. Выполнялось по три запуска выполнения тестов на каждое число N, после чего высчитывалось среднее значение времени выполнения алгоритма. Таблица с результатами замеров приведена ниже:
